...
# Some edits where done to this reference implementation, but only to transfer it to python 3, they're commented on
...
def DoDiv():
    a = Pop()
    b = Pop()
    # Push(b / a)  # This was changed from the reference implementation, python 3 changed when integer division is used
    Push(b // a)
...
def DoCmp():
    a = Pop()
    b = Pop()
    # This was added to the reference implementation because cmp no longer exists in python 3
    # replacement for cmp suggested at: https://docs.python.org/3.0/whatsnew/3.0.html
    cmp = lambda a, b: (b > a) - (b < a)
    Push(cmp(b, a))
...
try:
    ...
    sys.stderr.write(
        '!ERROR: exception while executing I=' + str(op_code) + ' PC=' + str(ProgramCounter - 1) + ' STACK_SIZE=' + str(
            len(OperandStack)) + '\n')
